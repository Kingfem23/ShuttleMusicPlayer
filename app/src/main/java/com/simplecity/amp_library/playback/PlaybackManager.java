...
public class PlaybackManager {
    ...
    private void openCurrentAndMaybeNext(boolean openNext) {
        ...
        if (openNext) {
            setNextTrack();
            Log.d("BiographyDialog", "setNextTrack is initiated");
            queueManager.getCurrentSong().add_play_count();
        }
    }
    ...
    public boolean open(@NonNull Song song) {
        ...
        queueManager.getCurrentSong().add_play_count();
        Log.d("BiographyDialog", "add_play_count is initiated2");
        stop(true);
        return false;
    }
        ...
        disposables.add(DataManager.getInstance().getSongsObservable(predicate)
                ...
                .subscribe(songs -> {
                    if (!songs.isEmpty() && queueManager.getCurrentSong() != null) {
                        ...
                        queueManager.getCurrentSong().add_play_count();
                        Log.d("BiographyDialog", "add_play_count is initiated");
                        System.out.println("Damn: " + queueManager.getCurrentSong().get_play_count());
                    }
    ...
    public void next(boolean force) {
        ...
        if(pos == 0){
            queueManager.promptShuffleList();
        }
        if (pos < 0) {
            setIsSupposedToBePlaying(false, true);
            return;
